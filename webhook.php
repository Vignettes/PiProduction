...
exec('sudo -u www-data git -C /var/www/html/PiProduction add -A 2>&1', $outputAdd, $returnAdd);
exec('sudo -u www-data git -C /var/www/html/PiProduction commit -m "Automated commit from webhook" 2>&1', $outputCommit, $returnCommit);
exec('sudo -u www-data git -C /var/www/html/PiProduction pull --rebase 2>&1', $outputPull, $returnPull);
exec('sudo -u www-data git -C /var/www/html/PiProduction push origin main 2>&1', $outputPush, $returnPush);

file_put_contents('/tmp/git_debug_log', implode("\n", array_merge($outputAdd, $outputCommit, $outputPull, $outputPush)));
